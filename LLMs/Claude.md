# Anthropic Claude System Prompt (Sonnet 4.5)

> **Source**: [claude-source.txt](../source/claude-source.txt)

## 1. Context & Overview
This is the system prompt for **Claude 3.5 Sonnet** (labeled as 4.5). It emphasizes **Safety**, **Artifact** usage, and a natural, non-robotic tone.

## 2. Prompt Breakdown

### The Anatomy of this System Prompt
Anthropic's prompt is famous for its **Safety & Constitution** approach. It is structured as a series of behavioral constraints and capability definitions, written in very natural, coherent English.

### Key Components
1.  **Identity & Humble Tone**:
    *   **What it is**: Instructions to be "Claude", with a tone that is "natural", "non-robotic", and devoid of "preachy" language.
    *   **Why it's included**: To differentiate from competitors. Claude is tuned to avoid the "As an AI language model..." repetitive refusal style, opting for more conversational refusals or direct answers.

2.  **Artifacts**:
    *   **What it is**: The `<antArtifact>` tags and rules.
    *   **Why it's included**: UX functionality. This instructs the model to separate substantial content (code, documents, SVGs) into a dedicated UI window, improving readability and allowing for versioning/downloading.

3.  **Visual & Safety Constraints**:
    *   **What it is**: Strict rules about not identifying people in images and not interacting with hidden text/images.
    *   **Why it's included**: Responsibility. It prevents the model from violating privacy or being "jailbroken" by hidden prompt injections in images.

## 3. How to Use This in Your Prompting
*   **Humanize Your Instructions**: Anthropic writes prompts in natural, conversational paragraphs. Instead of rigid bullet points for everything, try explaining your goal to the model like you would to a smart intern.
*   **Define Output Containers**: This prompt invents `<antArtifact>` tags to contain code. You can do the same! Tell Claude: "Put all summary points in `<summary>` tags and all action items in `<action>` tags." This makes parsing the response executable much easier.
*   **Pre-handle Refusals**: The prompt explicitly tells Claude *how* to refuse (don't be preachy). You can instruct it to "If you simply cannot do task X, explain why briefly and offer alternative Y," preventing the frustrating "I'm sorry, as an AI..." loops.
