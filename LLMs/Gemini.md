# Google Gemini System Prompt (AI Studio Vibe Coder)

> **Source**: [gemini-source.txt](../source/gemini-source.txt)

## 1. Context & Overview
This system prompt is designed for **Gemini** in a coding context, specifically acting as a "world-class senior frontend React engineer." features strict XML output and an opinionated stack (React 18+, Tailwind).

## 2. Prompt Breakdown

### The Anatomy of this System Prompt
Unlike general-purpose conversational prompts, this is a **Persona-Driven Coding Prompt**. It transforms the generic Gemini model into a specific product: the "Vibe Coder" for Google's AI Studio.

### Key Components
1.  **The "Senior Engineer" Persona**:
    *   **What it is**: Explicit instruction to act as a "world-class senior frontend React engineer."
    *   **Why it's included**: Priming. By assigning a high-competence role, the model accesses a more sophisticated subset of its training data (e.g., using better design patterns rather than basic tutorials).

2.  **Opinionated Tech Stack**:
    *   **What it is**: Strict mandates to use **React**, **Tailwind CSS**, **Framer Motion**, and **Lucide React**.
    *   **Why it's included**: Consistency. It prevents the model from randomly choosing between CSS-in-JS, Bootstrap, or vanilla CSS, ensuring the user gets a predictable "Vibe" that works immediately in the provided sandbox.

3.  **Self-Documentation (API Awareness)**:
    *   **What it is**: A massive chunk of the prompt defines *its own* API (Gemini API, Imagen, Veo).
    *   **Why it's included**: To minimize hallucinations. Since new model features (like 'Veo' video generation) might be newer than the model's training cut-off, the prompt "patches" the knowledge base with the correct, current code examples.

## 3. How to Use This in Your Prompting
*   **Prime the Model**: Notice how the prompt starts with "Act as a world-class senior frontend React engineer". You should always start your complex queries with a similar high-level role definition.
*   **Be Opinionated**: This prompt doesn't ask "which CSS framework should I use?". It *demands* Tailwind. In your own prompts, making decisions *for* the model (e.g., "Use Python 3.10, Pandas, and Plotly") yields much better results than letting it guess.
*   **Provide "Knowledge Patches"**: If you are using a library release from last week, the model won't know it. Copy-paste the library's "Quick Start" or API docs into your prompt just like Google did here for the `veo` model.
