# Cursor IDE System Prompt (Agent 2.0)

> **Source**: [cursor-source.txt](../source/cursor-source.txt)

## 1. Context & Overview
Cursor's prompt focuses on **Tool Use** (`codebase_search`, `grep`) and maximizing **Context**. It treats the AI as a pair programmer integrated into the editor.

## 2. Prompt Breakdown

### The Anatomy of this System Prompt
This is a **Tool-Use Heavy** prompt. The "intelligence" of the Cursor IDE doesn't come just from the model, but from the model's ability to drive the IDE's search and read features.

### Key Components
1.  **Context Management**:
    *   **What it is**: Instructions on how to assume context. "You can see the active file," "You cannot see the whole repo at once."
    *   **Why it's included**: Efficiency. It teaches the model *not* to hallucinate that it knows the whole project, prompting it to use search tools first.

2.  **Tool Definition (`codebase_search`, `grep`)**:
    *   **What it is**: Detailed definitions of how to fuzzy search or regex search the file system.
    *   **Why it's included**: Retrieval Augmented Generation (RAG). By giving the model a search engine for code, it can answer questions about functions defined in files it hasn't read yet.

3.  **Citation & Formatting**:
    *   **What it is**: Strict rules on how to format paths and cite code blocks.
    *   **Why it's included**: UI Integration. The Cursor app parses these citations to create clickable links and "Apply" buttons in the editor interface.

## 3. How to Use This in Your Prompting
*   **Context is King**: Cursor's prompt spends huge effort defining *what* it can see. When prompting manually, always explicitly paste or reference the files relevant to your question. Don't assume the model "knows" your project structure.
*   **Teach Search Strategies**: The prompt explains *how* to search (start broad, then narrow). You can use this logic effectively: "First, list all files that might be relevant. Then, plan which ones we need to read to fix this bug."
*   **Enforce Citations**: If you want to trust the code explanation, ask the model to "Cite the file path and line number for every claim." This forces it to hallucinate less and verify more.
